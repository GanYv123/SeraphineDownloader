...
void Downloader::StartDownload(const std::string &url, const std::string &savePath, AppLogic &logic)
{
    ...
    logic.SubmitTask(
        [this, url, savePath]()
        {
            ...
            {
                if (m_logCallback)
                    m_logCallback(std::string("Task exception: ") + e.what(), 2);
            }
            catch (...)
            {
                if (m_logCallback)
...
std::vector<std::string> Downloader::FetchFileList(const std::string &url)
{
    ...
    try
    {
        ...
        {
            if (m_logCallback)
                m_logCallback("Failed to parse URL", 2);
            return files;
        }

        HINTERNET hSession =
            WinHttpOpen(L"MyDownloader/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, NULL, NULL, 0);
        ...
        {
            if (m_logCallback)
                m_logCallback("Failed to send request", 2);
            return files;
        }
    ...
    {
        if (m_logCallback)
            m_logCallback(std::string("Exception in FetchFileList: ") + e.what(), 2);
    }
...
void Downloader::DownloadThread(const std::string &url, const std::string &savePath)
{
    try
    {
        ...
        while (WinHttpReadData(hRequest, buffer, sizeof(buffer), &dwDownloaded) && dwDownloaded > 0)
        {
            ...
            if (totalDownloaded - lastLogged > 50 * 1024 ||
                std::chrono::duration_cast<std::chrono::milliseconds>(now - lastTime).count() > 500)
            {
        ...
        m_success = true;
        if (m_logCallback)
            m_logCallback("Download complete: " + savePath, 1); // Success
    ...
    {
        if (m_logCallback)
            m_logCallback(std::string("Exception: ") + e.what(), 2);
    }
    catch (...)
    {
        if (m_logCallback)
            m_logCallback("Unknown exception occurred", 2);
    }
}
