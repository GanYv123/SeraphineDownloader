...
// ----------------- 快捷方式（支持传入文件夹） -----------------
bool FileManager::CreateShortcut(const std::wstring& folderPath, const std::wstring& shortcutName, LogCallback callback)
{
    ...
    if(!std::filesystem::exists(targetDir) || !std::filesystem::is_directory(targetDir)){
        if(callback) callback(u8"[ERROR] 文件夹不存在或不是目录", 2);
        return false;
    }
    ...
    if(exeFile.empty()){
        if(callback) callback(u8"[ERROR] 文件夹中没有 exe 文件", 2);
        return false;
    }
    ...
    if(!SUCCEEDED(SHGetFolderPath(NULL, CSIDL_DESKTOP, NULL, 0, desktopPath))){
        CoUninitialize();
        if(callback) callback(u8"[ERROR] 获取桌面路径失败", 2);
        return false;
    }
...
bool FileManager::RunProgram(const std::wstring& folderPath, LogCallback callback)
{
    ...
    if(callback) callback(u8"[INFO] 检查目录: " + targetDir.u8string(), 1);
    ...
    if(callback) callback(u8"[INFO] 找到可执行文件: " + exeFile.u8string(), 1);

    // ---------- 尝试 CreateProcess ----------
    STARTUPINFO si = { sizeof(si) };
    PROCESS_INFORMATION pi{};

    if(!CreateProcessW(exeFile.wstring().c_str(), nullptr, nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi)){
        DWORD errCode = GetLastError();

        if(errCode == ERROR_ELEVATION_REQUIRED){
            if(callback) callback(u8"[WARN] 程序需要管理员权限，尝试使用 ShellExecute 提权", 1);
            ...
            if((INT_PTR)hInst <= 32){
                if(callback) callback(u8"[ERROR] ShellExecute 提权启动失败", 2);
                return false;
            }

            if(callback) callback(u8"[INFO] 已通过 ShellExecute 启动程序（无法跟踪进程句柄）", 1);
        ...
        if(callback) callback(u8"[ERROR] 启动程序失败 (" + std::to_string(errCode) + "): " + utf8msg, 2);
        return false;
    }
    ...
    if(callback) callback(u8"[INFO] 程序启动成功 (PID=" + std::to_string(m_processId) + ")", 1);
    return true;
}
...
bool FileManager::CloseProgram(LogCallback callback)
{
    ...
    if(!m_processHandle){
        if(callback) callback(u8"[WARN] 程序未运行或是 ShellExecute 启动的（不可控）请手动关闭", 1);
        return false;
    }
