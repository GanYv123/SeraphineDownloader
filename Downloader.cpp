...
Downloader::~Downloader() { if(m_thread.joinable()) m_thread.join(); }

void Downloader::StartDownload(const std::string& url, const std::string& savePath)
{
    ...
    m_thread = std::thread(&Downloader::DownloadThread, this, url, savePath);
}
